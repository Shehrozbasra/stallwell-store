...
export default function Storefront() {
  ...
  async function completeOrder() {
    // Deduct stock for each purchased item in the shared database
    await Promise.all(
      cartItems.map((item) => {
        const remaining = Math.max(0, item.qty_available - item.qty);
        return updateProduct(item.id, { qty: remaining });
      })
    );
    setCart({});
    setView("success");
  ...
  return (
    <div style={styles.app}>
      ...
      <main style={styles.main}>
        ...
        {view === "checkout" && (
          <Checkout items={cartItems} subtotal={subtotal} onBack={() => setView("cart")} onComplete={completeOrder} />
        )}
...
function Checkout({ items, subtotal, onBack, onComplete }) {
  ...
  function handlePay() {
    ...
    setProcessing(true);
    // Demo simulation — replaced by real Stripe Checkout in the next setup step.
    setTimeout(() => {
      setProcessing(false);
      onComplete();
    }, 1100);
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease", maxWidth: 640, margin: "0 auto" }}>
      ...
      <div style={styles.checkoutSection}>
        ...
        <input style={styles.input} type="text" placeholder="Street, city, postcode" value={address} onChange={(e) => setAddress(e.target.value)} />
      </div>
      <div style={styles.summaryBox}>
        <div style={styles.summaryRow}><span>Subtotal</span><span>{currency(subtotal)}</span></div>
      ...
      <div style={styles.payBlock}>
        <button style={{ ...styles.walletBtn, background: "#000", opacity: canPay ? 1 : 0.5 }} disabled={!canPay} onClick={handlePay}> Pay</button>
        <button style={{ ...styles.walletBtn, background: "#fff", color: "#1a1a1a", border: "1px solid #ddd", opacity: canPay ? 1 : 0.5 }} disabled={!canPay} onClick={handlePay}>G Pay</button>
        <div style={styles.dividerRow}><span style={styles.dividerLine} /><span style={styles.dividerText}>or pay by card</span><span style={styles.dividerLine} /></div>
        <button style={{ ...styles.primaryBtn, opacity: canPay ? 1 : 0.5, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }} disabled={!canPay} onClick={handlePay}>
          <CreditCard size={16} />{processing ? "Processing…" : `Pay ${currency(total)}`}
        </button>
        <p style={styles.secureNote}><Lock size={12} style={{ marginRight: 5, verticalAlign: -1 }} />Demo checkout — connect Stripe to take real payments.</p>
      </div>
    </div>
  );
}
